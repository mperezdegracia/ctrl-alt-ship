...
- `operation_id` identifies the aggregate whose timeline receives the event. It
  is required except for the global `call.rejected` event.
...
| --- | --- | --- |
| `call.routed` | A known caller is linked to an operation and persona. | `direction`, `persona`, `intent`, `operation_reference`, `counterparty_type` |
