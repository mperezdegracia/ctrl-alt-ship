...
const container = `ctrl-alt-ship-retention-${randomUUID()}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    sql(readFileSync(resolve(__dirname, "evidence-retention.sql"), "utf8"));
    console.log("Evidence retention SQL passed: tombstones/FK, expiry, late callbacks, retry state and permissions.");
  } finally {
