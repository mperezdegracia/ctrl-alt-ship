...
const container = `ctrl-alt-ship-escalation-${randomUUID()}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    sql(readFileSync(resolve(__dirname, "escalation-cancellation.sql"), "utf8"));
    console.log("Escalation cancellation SQL passed: scope, idempotency, preserved data, pending review and transfer boundary.");
  } finally {
