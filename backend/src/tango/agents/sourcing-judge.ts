...
The mandate is intentionally short: route, cap/currency and pickup windows. A selected quote with accepted_above_budget true is an explicitly accepted price exception; its outside verdict does not invalidate SQL's selection. All other mandate checks still apply. Null optional payment, validity, cargo and conditions are expected; do not ask for them or invent values. Zero minimum payment means no restriction, not cash payment. Only price is negotiable. SQL remains responsible for eligibility, round deadlines and creating the immutable booking. This context belongs exclusively to its round_id; never consider another round or an earlier booking as a candidate.
