-- Local pickup dates must use the offset in the verified mandate, not model
-- geography/DST guesses. No mandate, booking, caller ID or recipient is rewritten.
BEGIN;

CREATE OR REPLACE FUNCTION public.mandate_pickup_utc_offset(windows jsonb)
RETURNS text LANGUAGE sql IMMUTABLE STRICT SET search_path=public,pg_temp AS $$
  -- The schema stores offset timestamps, not an IANA timezone. Only derive a
  -- common offset when ALL endpoints agree; never guess across mixed offsets.
  SELECT CASE WHEN count(DISTINCT utc_offset)=1 AND bool_and(utc_offset IS NOT NULL)
    THEN min(utc_offset) ELSE NULL END
  FROM (
    SELECT CASE WHEN endpoint.value ~ 'Z$' THEN '+00:00'
      ELSE substring(endpoint.value FROM '([+-][0-9]{2}:[0-9]{2})$') END AS utc_offset
    FROM jsonb_array_elements(windows) w
    CROSS JOIN LATERAL (VALUES (w->>'start_at'),(w->>'end_at')) endpoint(value)
  ) offsets;
$$;
REVOKE ALL ON FUNCTION public.mandate_pickup_utc_offset(jsonb) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.mandate_pickup_utc_offset(jsonb) TO service_role;
...
-- Replace only the private implementation. The public scope/selection wrapper,
-- immutable successor bookings, stale-context checks and receipt replay remain.
