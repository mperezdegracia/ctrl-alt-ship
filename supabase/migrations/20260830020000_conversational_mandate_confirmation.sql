-- Simplified conversational mandate confirmation. Keep prior migrations immutable.
-- Retain historical confirmation_evidence values; new mandates leave it NULL.
BEGIN;
...
BEGIN
  ...
  IF p_tool_name = 'confirm_mandate' THEN
    ...
    INSERT INTO public.mandates (
      ...
      (p_arguments->>'minimum_payment_term_days')::numeric::integer, c.id, confirmed_time, snapshot
...
COMMENT ON COLUMN public.mandates.confirmation_evidence IS 'Legacy audio evidence, retained for historical records; not populated by conversational confirmation.';
