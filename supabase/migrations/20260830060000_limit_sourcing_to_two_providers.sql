-- Keep already-deployed databases aligned with the two-provider demo limit.
...
BEGIN
  ...
  FOR candidate IN
    ...
    LIMIT 2
  LOOP
    INSERT INTO public.quote_requests (operation_id, provider_id, contact_attempt, status, expires_at, idempotency_key)
    VALUES (op.id, candidate.id, 1, 'queued', clock_timestamp() + interval '5 minutes',
