...
import { getDashboardHandoffs, getDirectoryEntries } from "@/lib/dashboard-api";
...
export default async function DirectoryPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const kind = first(query.kind) === "providers" ? "providers" : "contacts";
  ...
  return (
    <main className="dashboard-shell">
      ...
      <section className="dashboard-main dispatch-spine">
        <header className="dashboard-title-row"><div><h1>Directory</h1><p>Editable contact and provider records. Deactivation preserves the durable history.</p></div><Link className="refresh-link" href={`/dashboard/directory?${baseParams.toString()}`}>Reset directory</Link></header>
        ...
        <section className="ledger-section" aria-labelledby="directory-heading">
          <div className="operations-heading"><div><h2 id="directory-heading">{kind === "contacts" ? "Contacts" : "Providers"} <span>{directory.pagination.total}</span></h2><p>Counterparty phone numbers stay unique across both registers.</p></div><div className="system-view-tabs"><Link href={contactsHref} aria-current={kind === "contacts" ? "page" : undefined}>Contacts</Link><Link href={providersHref} aria-current={kind === "providers" ? "page" : undefined}>Providers</Link></div></div>
          <form className="operations-control-bar" action="/dashboard/directory" method="get">
            <input type="hidden" name="kind" value={kind} />
            <div className="operation-search"><label htmlFor="directory-search">Search {kind}</label><input id="directory-search" name="q" type="search" defaultValue={q} placeholder="Name, phone or email" /><button type="submit">Search</button></div>
          ...
          <DirectoryManager kind={kind} entries={directory.items} />
